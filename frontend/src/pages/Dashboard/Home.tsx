  ...
  return (
    <div className="flex flex-col gap-6 px-4 pt-6 pb-4 max-w-2xl mx-auto w-full transition-colors duration-200">
      ...
      <div className="bg-spendly-800 dark:bg-dark-surface text-white rounded-2xl p-5 transition-colors duration-200">
        {loadingSummary ? (
          <p className="text-spendly-200 dark:text-dark-muted text-sm">
            Loading...
          </p>
        ) : (
          <>
            <p className="text-sm text-spendly-200 dark:text-dark-muted">
              This month
            </p>
      ...
      {/* Top categories */}
      <div>
        <p className="text-sm font-medium text-spendly-700 dark:text-dark-muted mb-3">
          Top categories
        </p>
        {loadingSummary ? (
          <p className="text-sm text-spendly-700 dark:text-dark-muted">
            ...
            {(summary?.top_3_categories ?? []).map((cat) => (
              <div key={cat.name} className="flex flex-col gap-1">
                <div className="flex justify-between text-sm">
                  <span className="text-spendly-900 dark:text-dark-text">
                    {cat.name}
                  </span>
                  <span className="font-medium text-spendly-900 dark:text-dark-text">
                    ${fmt(cat.total)}
                  </span>
                </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-medium text-spendly-700 dark:text-dark-muted">
            Recent
          </p>
          <Link
            to="/expenses"
